...
class SlidingWindowSpec(AttentionSpec):
    ...
    def max_memory_usage_bytes(self, vllm_config: VllmConfig) -> int:
        ...
        # group is a tiny fixed 128-token window (layers 0/1/43), so we keep it
        # REPLICATED across DCP ranks instead of asserting dcp==1: the unsharded
        # per-rank size below is exactly the replicated footprint, and
        # block_table.py is told dcp_world_size=1 for this group so its slots
        # are not round-robin sharded. Gated on VLLM_SM86_DCP so upstream
        # sliding-window models keep the original hard guard.
        import os as _os
        if _os.environ.get("VLLM_SM86_DCP", "").strip() not in ("1", "true", "True"):
