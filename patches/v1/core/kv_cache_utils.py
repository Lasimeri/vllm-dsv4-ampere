...
def resolve_kv_cache_block_sizes(
    ...
    import os as _os
    _sm86_dcp = _os.environ.get("VLLM_SM86_DCP", "").strip() in ("1", "true", "True")
    if (dcp != 1 or pcp != 1) and not _sm86_dcp:
        raise ValueError(
    ...
    # Under context parallelism each group shards its KV round-robin by
    # (its block_size * dcp * pcp), so the scheduler's token-alignment invariant
    # must be LCM(block sizes) * dcp * pcp. This keeps the hybrid groups (and
    # their 4x/128x compression) intact while allowing CP -- unlike
    # --disable-hybrid-kv-cache-manager which unifies to the max page size and
    # destroys the compression. (VLLM_SM86_DCP: DeepseekV4-sparse hybrid CP.)
