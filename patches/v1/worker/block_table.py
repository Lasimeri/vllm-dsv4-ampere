...
class BlockTable:
    def __init__(
        ...
        try:
            ...
            self.dcp_rank = 0
        # DCP-exempt groups (e.g. DeepseekV4 sliding-window layers) stay
        # REPLICATED across DCP ranks: force world=1/rank=0 so compute_slot_mapping
        # stores every token locally instead of round-robin sharding it.
        if not shard_dcp:
            self.dcp_world_size = 1
            self.dcp_rank = 0
...
class MultiGroupBlockTable:
    ...
    def __init__(
        ...
        # Per-group DCP exemption: exempt groups (replicated across DCP ranks)
        # are not round-robin sharded and hold the full sequence locally.
        ...
        if max_num_blocks is None:
            ...
            # must be multiplied by dcp_world_size. Exempt (replicated) groups
            # hold the full sequence, so they divide by 1.
            total_cp_world_size = get_total_cp_world_size()
            max_num_blocks = [
                cdiv(max_model_len, block_size * (1 if exempt else total_cp_world_size))
                for block_size, exempt in zip(block_sizes, dcp_exempt)
            ]
        ...
        self.block_tables = [
            ...
            for block_size, kernel_block_size, max_num_blocks_per_req, exempt in zip(
                block_sizes, kernel_block_sizes, max_num_blocks, dcp_exempt
            )
        ]
