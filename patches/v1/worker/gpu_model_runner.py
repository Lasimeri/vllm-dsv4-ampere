...
class GPUModelRunner(
    ...
    def may_reinitialize_input_batch(
        ...
        block_sizes = []
        max_num_blocks = []
        # Per-group DCP exemption: sliding-window groups cannot be round-robin
        # sharded across DCP ranks (the window would be split), so they stay
        # REPLICATED. They divide their block count by 1, not the CP world size.
        # SlidingWindowMLASpec subclasses SlidingWindowSpec, so this covers both.
        ...
        for kv_cache_group in kv_cache_config.kv_cache_groups:
            ...
            exempt = isinstance(kv_cache_group.kv_cache_spec, SlidingWindowSpec)
            dcp_exempt.append(exempt)
            max_num_blocks_per_req = cdiv(
                max_model_len,
                block_size * (1 if exempt else get_total_cp_world_size()),
            )
