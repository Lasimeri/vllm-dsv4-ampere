...
if _is_flashmla_available()[0]:
    ...
    # SM86 (Ampere): vllm._flashmla_C is unavailable, but the pure-PyTorch
    # reference interface provides every entry point. Route to it when active.
    try:
        from vllm.third_party.flashmla.flash_mla_interface import (
            _flashmla_use_sm86_reference,
        )

        _use_sm86_fmla = current_platform.is_cuda() and _flashmla_use_sm86_reference()
    except Exception:
        _use_sm86_fmla = False
