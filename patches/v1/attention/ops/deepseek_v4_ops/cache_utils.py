...
def _quantize_and_insert_k_cache_pyref(
    k: torch.Tensor,
    k_cache: torch.Tensor,
    slot_mapping: torch.Tensor,
    block_size: int,
) -> None:
    """Vectorized: per-token Python loop replaced with batched scatter."""
    TOKEN_FP8_DIM = 448
    TOKEN_BF16_DIM = 64
    TOKEN_SCALE_DIM = 8
    QUANT_BLOCK_SIZE = 64
    ...
    fp8_part = k_v[:, :TOKEN_FP8_DIM].to(torch.float32).view(
        Nv, N_QUANT_BLOCKS, QUANT_BLOCK_SIZE
    )
    absmax = fp8_part.abs().amax(dim=-1).clamp_min(1e-4)
    exponent = torch.ceil(torch.log2(absmax / FP8_MAX))
    scale = torch.pow(2.0, exponent)
    x_quant = torch.clamp(
        fp8_part / scale.unsqueeze(-1), -FP8_MAX, FP8_MAX
    ...
    data_payload = torch.cat([x_quant_bytes, bf16_bytes], dim=1)  # [Nv, 576]

    scale_encoded = (exponent + 127.0).clamp(0.0, 255.0).to(torch.uint8)
    scale_payload = torch.zeros(
        Nv, TOKEN_SCALE_DIM, dtype=torch.uint8, device=device
    )
    ...
    scale_base_off = block_size * TOKEN_DATA_SIZE
    scale_base = (
        block_idx * block_stride + scale_base_off + pos_in_block * TOKEN_SCALE_DIM
    )
...
def _dequantize_and_gather_k_cache_pyref(
    out: torch.Tensor,
    k_cache: torch.Tensor,
    ...
    """Vectorized SM86 reference for dequantize_and_gather_k_cache.

    Per-block layout (block_stride bytes total):
      [0 : block_size * 576]  per-token data:
                                [0:448]   fp8 (uint8 storage, 7×64 groups)
                                [448:576] 64 bf16 (128 bytes)
      [block_size * 576 : ...] per-token UE8M0 scales (8 bytes; 7 used)
    """
    TOKEN_FP8_DIM = 448
    TOKEN_BF16_DIM = 64
    TOKEN_SCALE_DIM = 8
    ...
    seq_lens_l = seq_lens.long()
    gls = (
        gather_lens.long() if gather_lens is not None else seq_lens_l
    )
    starts = seq_lens_l - gls

    N_total = int(gls.sum().item())
    if N_total == 0:
        return

    # Build flat (req, within-req-token) indices via repeat + cumsum trick.
    cum = torch.zeros(num_reqs + 1, dtype=torch.long, device=device)
    cum[1:] = gls.cumsum(0)
    req_repeats = torch.repeat_interleave(
        torch.arange(num_reqs, device=device), gls
    )
    tok_idx_in_req = (
        torch.arange(N_total, device=device) - cum[:-1].repeat_interleave(gls)
    )

    starts_repeats = torch.repeat_interleave(starts, gls)
    pos = starts_repeats + tok_idx_in_req
    cache_block_size = k_cache.shape[1]
    block_in_seq = pos // cache_block_size
    pos_in_block = pos % cache_block_size
    phys_block = block_table[req_repeats, block_in_seq].long()
    ...
    arange_data = torch.arange(TOKEN_DATA_SIZE, device=device)
    arange_scale = torch.arange(N_QUANT_BLOCKS, device=device)

    data_base = phys_block * block_stride + pos_in_block * TOKEN_DATA_SIZE
    data_idx = (data_base.unsqueeze(-1) + arange_data).flatten()
    data_bytes = cache_flat_1d[data_idx].reshape(N_total, TOKEN_DATA_SIZE)
    ...
    scale_idx = (scale_base.unsqueeze(-1) + arange_scale).flatten()
    scale_bytes = cache_flat_1d[scale_idx].reshape(N_total, N_QUANT_BLOCKS)

    fp8_bytes = data_bytes[:, :TOKEN_FP8_DIM].contiguous()
    bf16_bytes = data_bytes[
        :, TOKEN_FP8_DIM : TOKEN_FP8_DIM + TOKEN_BF16_DIM * 2
    ].contiguous()
    ...
    fp8_grp = fp8_vals_bf.view(
        N_total, N_QUANT_BLOCKS, QUANT_BLOCK_SIZE
    ) * scales_bf.unsqueeze(-1)
    fp8_dequant = fp8_grp.reshape(N_total, TOKEN_FP8_DIM)

    out_token_idx = offset + tok_idx_in_req
    out[req_repeats, out_token_idx, :TOKEN_FP8_DIM] = fp8_dequant
    out[
        req_repeats, out_token_idx, TOKEN_FP8_DIM : TOKEN_FP8_DIM + TOKEN_BF16_DIM
    ] = bf16_vals
