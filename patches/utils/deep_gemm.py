...
def _fp8_einsum_pyref(equation, a_pair, b_pair, out, recipe=None):
    ...
    def _ensure_shape(operand, subs, sizes):
        ...
        if operand.dim() > len(subs):
            raise ValueError(
                f"operand has {operand.dim()} dims, equation subscripts {subs} expect {len(subs)}"
            )
        ...
        raise ValueError(
            f"cannot reshape operand {operand.shape} to match subscripts {subs} (known sizes: {sizes})"
        )
...
def fp8_fp4_mqa_logits(
    ...
    if _use_sm86_reference():
        ...
        return _fp8_mqa_logits_pyref(
            q_values, k_packed, k_scales, weights,
