...
def _w8a8_triton_block_scaled_mm_func(
    ...
    # SM86 fix: installed Triton's dtype dict has no 'float8_e8m0fnu' entry,
    # so any E8M0 scale tensor crashes the JIT binder. Upcast to fp32; the
    # value is preserved because E8M0 stores 2^e directly.
    e8m0 = getattr(torch, "float8_e8m0fnu", None)
    if e8m0 is not None:
        if x_scale.dtype == e8m0:
            x_scale = x_scale.to(torch.float32)
        if weight_scale.dtype == e8m0:
