...
def deepseek_v4_o_proj_bf16_sm86(
    o_fp8: torch.Tensor,
    ...
    # bf16 o-projection for AutoRound (W4A16) checkpoints where wo_a is bf16
    # (no fp8 scale). Opaque op so PIECEWISE splits here and the fp8->bf16
    # cast runs eagerly (sm_86 Triton cannot codegen fp8e4nv).
    dd = o_fp8.shape[-1]
    o_sf = o_scale.repeat_interleave(dd // o_scale.shape[-1], dim=-1)
    o_bf16 = o_fp8.to(torch.bfloat16) * o_sf.to(torch.bfloat16)
    wo_a_3d = wo_a_weight.to(torch.bfloat16).view(n_groups, o_lora_rank, dd)
    out.copy_(torch.einsum("bhr,hdr->bhd", o_bf16, wo_a_3d))


def deepseek_v4_o_proj_bf16_sm86_fake(
    o_fp8: torch.Tensor,
    o_scale: torch.Tensor,
    wo_a_weight: torch.Tensor,
    out: torch.Tensor,
...
direct_register_custom_op(
    op_name="deepseek_v4_o_proj_bf16_sm86",
