...
def _mhc_pre_big_fuse_pyref(
    ...
    """SM86 reference for mhc_pre_big_fuse_tilelang. Writes outputs in-place.

    Mirrors the TileLang fused kernel:
      • Reduce gemm_out_sqrsum / gemm_out_mul across the n_splits axis.
      • RMS-normalize the reduced mixes.
      • Compute pre_mix (sigmoid+eps), post_mix (sigmoid*mult).
      • Sinkhorn-iterate the comb block.
      • Apply pre_mix to residual to produce layer_input.
    """
    sqrsum = gemm_out_sqrsum.sum(dim=0)  # [N]
    rms = torch.rsqrt(sqrsum / (hc_mult * hidden_size) + rms_eps)  # [N]
    mixes = gemm_out_mul.sum(dim=0) * rms.unsqueeze(-1)  # [N, hc_mult3]

    pre_mix = torch.sigmoid(
        mixes[:, :hc_mult] * hc_scale[0] + hc_base[:hc_mult]
    ) + hc_pre_eps  # [N, hc_mult]
    ...
    cm = cm_flat.view(-1, hc_mult, hc_mult)
    ...
    comb_mix.copy_(cm.reshape(-1, hc_mult * hc_mult))

    li_f32 = (pre_mix.unsqueeze(-1) * residual.float()).sum(dim=1)
    layer_input.copy_(li_f32.to(torch.bfloat16))
...
def mhc_pre(
    ...
    from vllm.utils.deep_gemm import _use_sm86_reference, tf32_hc_prenorm_gemm
...
def _mhc_post_pyref(
    ...
    """SM86 reference for mhc_post_tilelang.

    out[n, j, h] = post[n, j] * x[n, h]
                 + sum_k comb[n, k, j] * residual[n, k, h]

    BF16 throughout — comb came from sinkhorn (values ~ 1/hc_mult ≈ 0.25)
    and post is sigmoid output, both fit BF16 dynamic range comfortably.
    Tensor cores fire on the matmul.
    """
    post = post_layer_mix.squeeze(-1).to(torch.bfloat16)  # [N, hc]
    x_bf = x.to(torch.bfloat16)  # [N, h]
    residual_bf = residual.to(torch.bfloat16)  # [N, hc, h]
    comb_bf = comb_res_mix.to(torch.bfloat16)  # [N, hc, hc]
    outer = post.unsqueeze(-1) * x_bf.unsqueeze(1)  # [N, hc, h]
    proj = torch.matmul(comb_bf.transpose(-1, -2), residual_bf)  # [N, hc, h]
    return (outer + proj).to(residual.dtype)
...
def mhc_post(
    ...
    if _use_sm86_reference():
        return _mhc_post_pyref(x, residual, post_layer_mix, comb_res_mix)
