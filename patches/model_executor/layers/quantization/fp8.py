...
class Fp8LinearMethod(LinearMethodBase):
    ...
    def process_weights_after_loading(self, layer: Module) -> None:
        if getattr(layer, "is_bmm", False) and self.use_marlin:
            # SM86: DeepseekV4 wo_a (is_bmm) is consumed by the
            # deepseek_v4_fp8_einsum pyref, which dequantizes the RAW block-fp8
            # weight itself ([out, in] + per-[128,128]-block scale; _ensure_shape
            # reshapes it to the grouped [g, d, r] the einsum needs). The default
            # SM86 fp8 path is Marlin, whose repack rewrites the weight into a
            # packed layout the einsum cannot reshape (observed: [1024,4096] ->
            # [256,4096], scale transposed -> crash). Leave the loaded block-fp8
            # weight + per-block scale untouched. (On Hopper, is_bmm goes through
            # the DeepGEMM kernel path below, not Marlin.)
