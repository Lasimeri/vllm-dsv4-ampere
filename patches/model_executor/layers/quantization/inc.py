...
class INCConfig(QuantizationConfig):
    ...
    def get_layer_config(self, layer, layer_name: str):
        def get_config(name: str, quantized: bool = True):
            ...
            # MTP draft layers are constructed with the prefix
            # `model.layers.{num_hidden_layers + i}.<suffix>` (the `mtp_block`
            # attribute is not part of the quant-resolution prefix), but their
            # per-module overrides live under the checkpoint's `mtp.{i}.<suffix>`
            # keys -- and the MTP model has no hf_to_vllm_mapper to bridge the
            # two. As a fallback (reached only when no explicit `layers.{N}` key
            # matched, i.e. the trailing MTP layer), match an `mtp.{i}.<suffix>`
            # key by its suffix so overrides like `mtp.0.attn.wo_a` (bf16) apply
            # to the draft head instead of defaulting to quantized. Safe because
            # every quantization-excluded main-model module carries its own
            # explicit `layers.{N}` key and matches earlier.
            m_layer = re.match(r"^(?:model\.)?layers\.\d+\.(.+)$", name)
            if m_layer is not None:
                name_suffix = m_layer.group(1)
                for pattern, cfg in self.extra_config.items():
                    if not isinstance(pattern, str):
                        continue
                    mk = re.match(r"^(?:model\.)?mtp\.\d+\.(.+)$", pattern)
                    if mk is not None and mk.group(1) == name_suffix:
                        return (
                            cfg.get("bits", self.weight_bits if quantized else 16),
                            cfg.get("group_size", self.group_size if quantized else -1),
                            cfg.get("sym", self.sym if quantized else True),
